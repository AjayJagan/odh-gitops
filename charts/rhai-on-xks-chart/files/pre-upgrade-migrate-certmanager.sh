...
ELAPSED=0
while [[ $ELAPSED -lt $WAIT_TIMEOUT ]]; do
  if ! REMAINING_PODS=$(kubectl get pods -n cert-manager-operator \
    -l name=cert-manager-operator \
    -o name); then
...
if [[ $ELAPSED -ge $WAIT_TIMEOUT ]]; then
  ...
  echo "Remaining pods:"
  kubectl get pods -n cert-manager-operator -l name=cert-manager-operator || true
  exit 1
fi
