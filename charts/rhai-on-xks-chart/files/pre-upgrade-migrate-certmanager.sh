...
if kubectl get deployment -n cert-manager-operator \
  -l infrastructure.opendatahub.io/part-of -o name 2>/dev/null | grep -q .; then
  ...
  elif ! kubectl get deployment -n cert-manager-operator \
    -l infrastructure.opendatahub.io/part-of -o name 2>/dev/null | grep -q .; then
